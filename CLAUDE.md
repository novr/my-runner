...
`jit-config.sh` は内部で `github-token.sh` を呼んでトークンを取得してから `generate-jitconfig` API を叩く。`encoded_jit_config` を VM 内の `./run.sh --jitconfig <value>` に渡すと、1ジョブ実行後にランナーが自動登録解除される。
